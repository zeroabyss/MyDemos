...
public class OverlayDemo extends Activity {
    ...
    *//**
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        MapStatusUpdate msu = MapStatusUpdateFactory.zoomTo(14.0f);//缩放等级为14
        mBaiduMap.setMapStatus(msu);//设置缩放等级
        initOverlay();
        mBaiduMap.setOnMarkerClickListener(new OnMarkerClickListener() {
            public boolean onMarkerClick(final Marker marker) {
                Button button = new Button(getApplicationContext()); //maker显示框控件 可以是view的任何一种
                button.setBackgroundResource(R.drawable.popup);      //设置控件图片
                OnInfoWindowClickListener listener = null;           //显示窗的监听
                if (marker == mMarkerA || marker == mMarkerD) {
                    button.setText("更改位置");
                    button.setBackgroundColor( 0x0000f );
                    button.setWidth( 300 );

                    listener = new OnInfoWindowClickListener() {        //写在markerA D判断条件内的原因可以使得listener少写判断条件；
